...
The Golden Loop is complete. Patterns have evolved based on this outcome.`,
      ...
      case 'get_context': {
        ...
        const effectiveUserId = userId || EKKOS_USER_ID || null;
      ...
      case 'create_plan': {
        ...
        const userId = EKKOS_USER_ID || null;
        const { data, error } = await supabase
          .from('agent_plans')
          .insert({
            title,
      ...
      case 'create_plan_from_template': {
        ...
        const result = await fetchMemory(`/api/v1/plans/${name.replace('_', '-')}`, {
          method: 'POST',
          body: JSON.stringify({ ...args, user_id: EKKOS_USER_ID || null })
        });
        return {
      ...
      case 'check_conflict': {
        ...
        const result = await fetchMemory('/api/v1/reflex/check', {
          method: 'POST',
          body: JSON.stringify({ proposed_action, scope, include_patterns, user_id: EKKOS_USER_ID || null })
        });
        return {
          content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
        };
      ...
      case 'store_secret': {
        ...
        const result = await fetchMemory('/api/v1/secrets/store', {
          method: 'POST',
          body: JSON.stringify({ service, value, type, description, expires_in_days: expiresInDays, user_id: EKKOS_USER_ID || null })
        });
        return {
          content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
        };
      ...
      case 'rotate_secret': {
        ...
        const result = await fetchMemory('/api/v1/secrets/rotate', {
          method: 'POST',
          body: JSON.stringify({ service, type, new_value: newValue, user_id: EKKOS_USER_ID || null })
        });
        return {
          content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
        };
      ...
      case 'detect_usage': {
        ...
        const result = await fetchMemory('/api/v1/learning/detect-usage', {
          method: 'POST',
          body: JSON.stringify({ response, retrieval_id, query, user_id: EKKOS_USER_ID || null })
        });
        return {
          content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
        };
      ...
      case 'search_codebase': {
        ...
        const result = await fetchMemory('/api/v1/codebase/search', {
          method: 'POST',
          body: JSON.stringify({ query, limit, file_types, user_id: EKKOS_USER_ID || null })
        });
        return {
          content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
        };
